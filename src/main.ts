...
import {researchFinding,renderExaResearch} from './exa.js';
...
try {
  ...
  const result=fixtureResult.status==='complete'&&modelResult.status==='failed'
    ...
    : modelResult;
  const exa=await researchFinding(result,{apiKey:process.env.EXA_API_KEY});
  stage='GitHub publication';
  await publishAssessment(config,result,renderAssessment(result,config.repository)+renderExaResearch(exa));
  ...
  console.log(`Exa Agent: ${exa.status}. ${exa.reason}`);
