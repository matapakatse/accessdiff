...
const evidenceSchema = {type:'array',items:{type:'object',additionalProperties:false,required:['revision','line','quote'],properties:{revision:{type:'string',enum:['base','head']},line:{type:'integer'},quote:{type:'string'}}}};
const outputSchema = {type:'object',additionalProperties:false,required:['status','statusReason','scenarios','finding'],properties:{status:{type:'string',enum:['complete','incomplete']},statusReason:{type:'string'},scenarios:{type:'array',items:{type:'object',additionalProperties:false,required:['actor','before','after','explanation','evidence'],properties:{actor:{type:'string',enum:['owner','other_customer','anonymous']},before:{type:'string',enum:['allowed','denied','unknown']},after:{type:'string',enum:['allowed','denied','unknown']},explanation:{type:'string'},evidence:evidenceSchema}}},finding:{anyOf:[{type:'null'},{type:'object',additionalProperties:false,required:['title','impact','suggestion','evidence'],properties:{title:{type:'string'},impact:{type:'string'},suggestion:{type:'string'},evidence:evidenceSchema}}]}}};
export async function assessModel(input:ReviewInput,options:{apiKey:string;model:string;fetch?:typeof fetch;timeoutMs?:number}):Promise<AssessmentResult>{
 ...
 try{const response=await(options.fetch??fetch)('https://openrouter.ai/api/v1/chat/completions',{method:'POST',signal:AbortSignal.timeout(options.timeoutMs??45000),headers:{Authorization:`Bearer ${options.apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model:options.model,max_tokens:4096,temperature:0,response_format:{type:'json_schema',json_schema:{name:'access_assessment',strict:true,schema:outputSchema}},messages:[
